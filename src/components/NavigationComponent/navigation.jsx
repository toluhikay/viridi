...
import Logo from "assets/NewAssets/ViridiLogo3.png";
	...
	return (
		<Fragment>
			...
			<div className='lg:px-24 sticky z-[999990000] top-0 left-0 right-0 bg-white md:px-12 px-6 lg:py-6 md:py-4 py-3 '>
				...
				<nav className={`w-2/3 bg-white transition-all flex flex-col justify-start items-start fixed h-screen top-0 ${!navOpen ? "left-[-150%]" : "left-0"}`}>
					<div>
						<div>
							<img className='w-[150px] mb-12 h-auto' src={Logo} alt='' />
						</div>
						<div></div>
					</div>
					<ul className={"h-[70%] pl-12 flex py-6 flex-col justify-between"}>
						{links.map((items) => {
							return (
								<Fragment>
									<li key={items.id}>
