...
const Footer = () => {
	...
	const links = [
		{ id: 1, link: "Fumigation and Pest Control", to: "fumigationServices" },
		{ id: 2, link: "Painting Services", to: "paintServices" },
		{ id: 2, link: "Janitorial Services", to: "janitorialServices" },
	];
				...
				<div className='w-[30%]'>
					...
					<div className='flex'>
						{socials.map((item) => {
							return (
								<Fragment>
									<a href={item.link} key={item.id} target='_blank' rel='noreferrer' className='mr-3 text-green-500 bg-[#c4c4c470] rounded-full p-2 hover:text-white'>
										{item.logo}
									</a>
								</Fragment>
							);
					...
					<ul className=''>
						{links.map((items) => {
							return (
								<Fragment>
									<li key={items.id}>
										<Link className=' capitalize mb-6 opacity-70 font-bold' to={items.to}>
											{items.link}
										</Link>
									</li>
								</Fragment>
